...
class ScienceFragment : Fragment(R.layout.fragment_science) {
    ...
    lateinit var itemScienceError: CardView
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        itemScienceError = view.findViewById(R.id.itemScienceError)
        ...
        setUpScienceRecycler()

        newsAdapter.setOnItemClickListener {
            val bundle = Bundle().apply {
                putSerializable("article",it)
            }
            findNavController().navigate(R.id.action_scienceFragment_to_articleFragment, bundle)
        }

        newsViewModel.scienceNews.observe(viewLifecycleOwner, Observer {response->
            when(response){
                is Resource.Success<*> -> {
                    hideProgressBar()
                    hideErrorMessage()
                    response.data?.let {newsResponse ->
                        ...
                        if (isLastpage){
                            binding.recyclerScience.setPadding(0,0,0,0)
                        }
                    }
                }
                is Resource.Error<*> -> {
        ...
        retryButton.setOnClickListener{
            newsViewModel.fetchScienceNews("in")
        }
    }
    ...
    private fun hideProgressBar(){
        binding.paginationProgressBarScience.visibility = View.INVISIBLE
        isLoading = false
    }

    private fun showProgressBar(){
        binding.paginationProgressBarScience.visibility = View.VISIBLE
        isLoading = true
    }

    private fun hideErrorMessage(){
        itemScienceError.visibility = View.INVISIBLE
        isError = false
    }
    private fun showErrorMessage(message:String){
        itemScienceError.visibility = View.VISIBLE
    ...
    private fun setUpScienceRecycler(){
        newsAdapter = NewsAdapters()
        binding.recyclerScience.apply {
            adapter = newsAdapter
